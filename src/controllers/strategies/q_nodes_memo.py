...
class QNodesMemo(SIA):
        ...
        for i in range(len(vertices_fase) - 2):
            ...
            for j in range(len(deltas_ciclo) - 1):
                ...
                for k in range(len(deltas_ciclo)):
                    ...
                    # Si encontramos una pérdida de 0, almacenamos y retornamos
                    if emd_delta == 0:
                        particion_optima = tuple([tuple(deltas_ciclo[k])])
                        # Almacenamos la partición óptima en memoria
                        self.memoria_particiones[particion_optima] = (emd_delta, dist_marginal_delta)
                        print(f"Partición óptima encontrada: {particion_optima} con EMD: {emd_delta}")
    ...
    def funcion_submodular(
        ...
        #Generar clave única para el delta
        delta_key = deltas if isinstance(deltas, tuple) else tuple(deltas)

        #Verificar si necesitamos calcular el delta
        if delta_key not in self.memoria_delta:
            #emd_delta = INFTY_NEG NO es necesario, ya que se calcula en la función
            #El valor de emd_delta se obtiene directamente del diccionario memoria_delta[key]
            #No necesita inicialización, pues si delta no está en memoria, se calcula con emd_efecto() y se asigna antes de guardarse en memoria_delta
            #La memoización garantiza que emd_delta siempre tendrá un valor válido

            #Preparar arrays temporales apra deltas
            temporal = [[], []]
        ...
        #Obtener valores del delta (de memoria o recién calculados)
        emd_delta, vector_delta_marginal = self.memoria_delta[delta_key]

        #Preparar configuración temporal para la unión (delta + omega)
        temporal_union = [[], []]

        #Procesar deltas para la unión
        if isinstance(deltas, tuple):
            d_tiempo, d_indice = deltas
            temporal_union[d_tiempo].append(d_indice)
        else:
            for delta in deltas:
                d_tiempo, d_indice = delta
                temporal_union[d_tiempo].append(d_indice)
        ...
        # Procesar omegas para la unión
        for omega in omegas:
            if isinstance(omega, list):
                for omg in omega:
                    o_tiempo, o_indice = omg
                    temporal_union[o_tiempo].append(o_indice)
            else:
                o_tiempo, o_indice = omega
                temporal_union[o_tiempo].append(o_indice)

        copia_union = self.sia_subsistema
        dims_alcance_union = temporal_union[EFECTO]
        dims_mecanismo_union = temporal_union[ACTUAL]
